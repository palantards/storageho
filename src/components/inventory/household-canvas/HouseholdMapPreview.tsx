  ...
  return (
    ...
    >
      <div className="text-sm font-medium">
        {tt("app.canvasSetup.previewTitle", "Read-only map preview")}: {floorName}
      </div>
      <div className="text-xs text-muted-foreground">
        {tt(
          "app.canvasSetup.previewHint",
          "Preview only in this version. Layout is auto-packed and non-overlapping.",
        )}
      </div>
                ...
                <div className="grid grid-cols-2 gap-2 xl:grid-cols-3">
                  {group.containers.map((container) => (
                    <button
                      key={container.id}
                      type="button"
                      className="rounded border bg-muted/30 px-2 py-2 text-left text-xs hover:border-primary/60"
                      onClick={() => onOpenBox?.(container.id)}
                    >
